...
export const ANALYSIS_ENTRIES: AnalysisEntry[] = [
  {
    ...
    tagline: "A record Nasdaq listing, a '35% crash' every headline ran with, 1.2 million margin calls, and a president's approval rating cracking 50% — the real chain of events, and the one headline number that doesn't survive a closer look",
    date: "2026-08-07",
    body: `# Korea's leverage crash: SK Hynix, the KOSPI, and what actually broke

I wrote the first version of this piece after four days of a story that looked done. It wasn't close to done. What started as a record Nasdaq listing turned, over the following four weeks, into South Korea's single worst month for equities on record — worse, by some measures, than any single month of the 1997 Asian Financial Crisis or 2008 — 1.2 million retail accounts hit with margin calls, a president's disapproval rating cracking 50% for the first time, and a market still whipsawing violently as of this week. This is the full version: the real timeline, the mechanics of how a brand-new financial product turned a normal correction into a crisis, and a check on the headline number itself. Every outlet covering this cites some version of a large SK Hynix drawdown — 30%, 35%, worse. None of them are fabricating anything; the underlying reporting on the leverage and the margin calls is solid. But a headline reaches for the biggest defensible number, not necessarily the most accurate one, and once I actually checked SK Hynix's own Nasdaq-listed stock against an honest starting point rather than the one that produces the biggest number, the real figure is smaller, and more interesting, than what's been reported.
...
## Two listings, one stock, two opposite stories — and why that matters for the number everyone's citing

Worth surfacing on its own, because it's the key to the next section: on 14 July — the day every headline cites as "the crash" — SK Hynix's Nasdaq ADR didn't fall. It closed at $193.92, up from $152.35 the day before — a **27% single-day gain** — on the same day the Seoul-listed common shares fell 15.37%.
...
That split matters for more than trivia. It means 14 July — the date most coverage, including my own earlier draft, treats as SK Hynix's pre-crash peak — wasn't a normal price level for the ADR at all. It was a temporary, IPO-driven spike sitting on top of the stock's actual trend. Measuring "the crash" from that spike inflates the number.

## Headlines reach for the biggest defensible number. Here's the honest one.

Every article on this crash cites some version of a large SK Hynix drawdown — reasonably, since the leverage, the margin calls, the forced liquidations behind it are all real and well-reported. But "SK Hynix's ADR crashed 35%" is the kind of number a headline reaches for because it's the biggest one defensibly available, measured from that inflated 14 July print. It isn't the most accurate description of what happened to the stock.

Index SK Hynix's ADR and Micron — a comparable, US-listed, unlevered memory-chip peer riding the same AI/HBM demand cycle, with zero exposure to Korea's leveraged-ETF market — to 100 on **10 July**, SKHY's actual first trading day (there's no earlier price history for this specific security; the window can't start any earlier than that), and run both through the 29 July trough: **SK Hynix's ADR fell 24.53%. Micron fell 24.54%.** Statistically the same stock.
...
What *is* real and genuinely, structurally Korean is everything that happened on the other listing: the 1.2 million margin calls, the 320,000+ forced liquidations, the political fallout — all of it ran through the Seoul-listed shares and Korea's leveraged-ETF plumbing, which is mechanically separate from the Nasdaq ADR. The leverage crisis is real. It just isn't what shows up in the ADR's own price chart, once you check it against an honest baseline instead of the number that makes the best headline.
...
`,
    chart: {
      ...
      annotations: [
        ...
        { date: "2026-07-14", label: "Seoul shares -15% same day ADR +27% — the split. This ADR spike, not Korea leverage, is why 7/14 overstates the later 'crash'" },
